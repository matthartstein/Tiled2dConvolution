...
int main(int argc, char* argv[])
{
    ...
    if(cuda_ret != cudaSuccess) FATAL("\nERROR: Unable to allocate device variables");
    stopTime(&timer); printf("%f s\n", elapsedTime(timer));
    ...
    cuda_ret = cudaDeviceSynchronize();
    if(cuda_ret != cudaSuccess) FATAL("\nERROR: Unable to copy host variables to device");
    stopTime(&timer); printf("%f s\n", elapsedTime(timer));
    ...
    //INSERT CODE HERE
	const int h_blocks = (imageHeight / TILE_SIZE + 1);
	const int w_blocks = (imageWidth / TILE_SIZE + 1);

    	dim_block = dim3(TILE_SIZE, TILE_SIZE);
	dim_grid = dim3(w_blocks, h_blocks);
